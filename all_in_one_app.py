...
try:
    ...
    kcs_encode_wav = kcs_decode_wav = kcs_load_cfg = kcs_config_global = kcs_get_filename_ext = kcs_adjust_frequencies = None
    kcs_initial_config = {}
...
try:
    ...
    from img2txt import (
        encode as img_encode, 
        decode as img_decode, 
        MODES as IMG_MODES
    )
except ImportError as e:
...
try:
    ...
    print("Please ensure replace_polish_signs.py is in the same directory or Python path.")
...
def get_valid_filepath(prompt_message, must_exist=True):
    ...
    while True:
        ...
        if not must_exist and os.path.isdir(filepath):
            print(f"Error: Expected a file path, but got a directory: '{filepath}'. Please try again.")
            continue
        return filepath
...
def get_output_filepath(input_path, default_suffix, default_ext):
    ...
    while True:
        user_path = input(f"Enter output file path (default: {suggested_path}): ").strip()
        if not user_path:
            return suggested_path
        
        # Basic validation: ensure it's not a directory if it exists
        if os.path.isdir(user_path):
            print(f"Error: Output path '{user_path}' is a directory. Please enter a valid file path.")
            continue
        
        # Ensure parent directory exists
        parent_dir = os.path.dirname(user_path)
        if parent_dir: # Only try to create if parent_dir is not empty (i.e., not saving in CWD)
...
def main():
    if not kcs_initial_config and (kcs_encode_wav or kcs_decode_wav): # Check if KCS part might be used
        print("Warning: KCS configuration could not be loaded. KCS features might not work.")
    ...
    while True:
        ...
        try: 
            if choice == '1': # KCS Encode
                if not kcs_encode_wav:
                    print("Error: KCS encode function is not available (import failed).")
                else:
                    print("\n--- Encode Text to KCS WAV ---")
                    input_file = get_valid_filepath("Enter path to the input text file: ", must_exist=True)
                    if input_file: # Proceed only if a valid input file was provided
                        output_file = get_output_filepath(input_file, "_encoded", ".wav")
                        if output_file: # Proceed only if a valid output file was provided/confirmed
                            print(f"Using KCS Config: ZERO_FREQ={kcs_config_global.get('ZERO_FREQ')}, ONES_MULT={kcs_config_global.get('ONES_MULT')}")
                            try:
                                success = kcs_encode_wav(input_file, output_file, kcs_config_global)
                                if success:
                                    print(f"Successfully encoded '{input_file}' to '{output_file}'")
                                else:
                                    # The module itself should log specifics if possible
                                    print(f"Error: Failed to encode '{input_file}'. Module indicated failure.")
                            except Exception as e:
                                print(f"Error: An exception occurred during KCS encoding operation.")
                                print(f"Debug details: {type(e).__name__} - {e}")

            elif choice == '2': # KCS Decode
                if not kcs_decode_wav:
                    print("Error: KCS decode function is not available (import failed).")
                else:
                    print("\n--- Decode KCS WAV to Text ---")
                    input_file = get_valid_filepath("Enter path to the KCS WAV file: ", must_exist=True)
                    if input_file:
                        ...
                        output_path_base_for_decode = os.path.join(output_dir, output_base_name_user)
                        
                        print(f"Using KCS Config: ZERO_FREQ={kcs_config_global.get('ZERO_FREQ')}, ONES_MULT={kcs_config_global.get('ONES_MULT')}")
                        try:
                            output_filename = kcs_decode_wav(input_file, output_path_base_for_decode, kcs_config_global)
                            if output_filename:
                                print(f"Successfully decoded '{input_file}' to '{output_filename}'")
                            else:
                                print(f"Error: Failed to decode '{input_file}'. Module indicated failure.")
                        except Exception as e:
                            print(f"Error: An exception occurred during KCS decoding operation.")
                            print(f"Debug details: {type(e).__name__} - {e}")
            
            elif choice == '3': # Image Encode
                if not img_encode or not IMG_MODES:
                    print("Error: Image encode function or modes are not available (import failed).")
                else:
                    print("\n--- Encode Image to Binary File ---")
                    input_file = get_valid_filepath("Enter path to the input image file: ", must_exist=True)
                    if input_file:
                        ...
                        selected_mode_name = IMG_MODES.get(mode_choice_input) # Check if input is a key like "1"
                        if not selected_mode_name: # If not a key, check if it's a value like "MONO8"
                            ...
                            if not is_value_match: # Default if invalid or empty
                                ...
                                if mode_choice_input: # User entered something, but it was invalid
                                    print(f"Info: Invalid mode selection '{mode_choice_input}'. Defaulting to {selected_mode_name}.")
                                else: # User entered nothing
                                     print(f"Info: No mode selected. Defaulting to {selected_mode_name}.")
                        print(f"Selected mode: {selected_mode_name}")
                        try:
                            output_file = img_encode(input_file, selected_mode_name)
                            if output_file:
                                print(f"Successfully encoded '{input_file}' to '{output_file}' using mode {selected_mode_name}")
                            else:
                                print(f"Error: Failed to encode '{input_file}'. Module indicated failure.")
                        except Exception as e:
                            print(f"Error: An exception occurred during image encoding operation.")
                            print(f"Debug details: {type(e).__name__} - {e}")

            elif choice == '4': # Image Decode
                if not img_decode:
                    print("Error: Image decode function is not available (import failed).")
                else:
                    print("\n--- Decode Binary File to Image ---")
                    input_file = get_valid_filepath("Enter path to the encoded binary file: ", must_exist=True)
                    if input_file:
                        try:
                            ...
                            print(f"Error: An exception occurred during image decoding operation.")
                            print(f"Debug details: {type(e).__name__} - {e}")

            elif choice == '5': # Replace Polish Signs
                if not process_polish_file:
                    print("Error: Polish signs processing function is not available (import failed).")
                else:
                    print("\n--- Remove Polish Signs from Text File ---")
                    input_file = get_valid_filepath("Enter path to the input text file: ", must_exist=True)
                    if input_file:
                        output_file = get_output_filepath(input_file, "_removed_PL", ".txt")
                        if output_file:
                            try:
                                success = process_polish_file(input_file, output_file, 'PL2')
                                if success:
                                    print(f"Successfully processed '{input_file}', Polish signs removed. Output: '{output_file}'")
                                else:
                                    print(f"Error: Failed to process '{input_file}'. Module indicated failure.")
                            except Exception as e:
                                print(f"Error: An exception occurred during Polish sign removal.")
                                print(f"Debug details: {type(e).__name__} - {e}")
                ...
                if not process_polish_file:
                    ...
                    input_file = get_valid_filepath("Enter path to the text file with removed Polish signs: ", must_exist=True)
                    if input_file:
                        output_file = get_output_filepath(input_file, "_restored_PL", ".txt")
                        if output_file:
                            try:
                                success = process_polish_file(input_file, output_file, '2PL')
                                if success:
                                    print(f"Successfully processed '{input_file}', Polish signs restored. Output: '{output_file}'")
                                else:
                                    print(f"Error: Failed to process '{input_file}'. Module indicated failure.")
                            except Exception as e:
                                print(f"Error: An exception occurred during Polish sign restoration.")
                                print(f"Debug details: {type(e).__name__} - {e}")
        ...
        except Exception as e: # Catch-all for truly unexpected errors in the main loop or input functions
            print(f"Critical Error: An unexpected issue occurred in the application: {e}")
            print(f"Debug details: {type(e).__name__} - {e}")
...
if __name__ == "__main__":
    ...
    if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
        os.chdir(sys._MEIPASS) # Change CWD to where bundled files are if running as PyInstaller bundle
    main()
