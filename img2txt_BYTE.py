...
def unpack_2bit_data(data, num_pixels):
    ...
    pixels = []
    num_bytes = (num_pixels + 3) // 4  # Number of bytes needed to store num_pixels (rounded up)

    if len(data) < num_bytes:
        raise ValueError(f"Insufficient data. Expected at least {num_bytes} bytes, but got {len(data)}.")
...
def unpack_4bit_data(data, num_pixels):
    ...
    pixels = []
    num_bytes = (num_pixels + 1) // 2  # Number of bytes needed to store num_pixels (rounded up)
...
def run_length_decode(encoded_data):
    ...
    while idx < len(encoded_data):
        ...
        count = int.from_bytes(encoded_data[idx + 1:idx + 5], 'big')  # Next 4 bytes are the count
